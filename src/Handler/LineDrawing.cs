...
namespace ScreenYu.Handler {
    internal class LineDrawing : EmptyHandler, IDispatcherHandler, IResetHandler {
        private Drawing.Line? ObjectInProgress;
        ...
        public override void MouseMove(Context.I ctx, MouseEventArgs e) {
            ...
            if ((Control.ModifierKeys & Keys.Shift) != 0) { // toggle perpendicular
                if (Math.Abs(e.X - ObjectInProgress.X1) >= Math.Abs(e.Y - ObjectInProgress.Y1)) { // horizontal
                    ObjectInProgress.X2 = e.X;
                    ObjectInProgress.Y2 = ObjectInProgress.Y1;
                }
                else { // vertical
                    ObjectInProgress.X2 = ObjectInProgress.X1;
